...
export default function TrackOrderPage() {
  ...
  return (
    <div className="animate-in fade-in duration-500">
      <h2 className="mb-8 border-b border-[#F0D9E5] pb-4 text-lg font-bold uppercase tracking-widest text-[#1A0A10]">
        Track Your Order
      </h2>

      <div className="mb-10 border border-[#F0D9E5] bg-[#FDF6F8] p-6">
        <form onSubmit={handleTrack} className="flex flex-col gap-4 md:flex-row">
          <div className="flex-1">
            <label className="mb-2 block text-[10px] font-bold uppercase tracking-widest text-[#8D6E7F]">Invoice ID</label>
            <input
              type="text"
              value={invoiceId}
              onChange={(e) => setInvoiceId(e.target.value)}
              placeholder="e.g. INV-12345"
              className="h-12 w-full border border-[#F0D9E5] bg-white px-4 text-sm transition-colors focus:border-[#C2185B] focus:outline-none"
            />
          </div>
          <div className="md:self-end">
            <button
              type="submit"
              disabled={loading}
              className="flex h-12 w-full items-center justify-center gap-2 bg-[#C2185B] px-8 text-xs font-bold uppercase tracking-widest text-white transition-colors hover:bg-[#9C0E47] disabled:opacity-50 md:w-auto"
            >
      ...
      {orderData && (
        <div className="border border-[#F0D9E5] bg-white p-4 md:p-8">
          <div className="mb-10 flex flex-wrap items-center justify-between gap-4 border-b border-[#F0D9E5] pb-6">
            <div>
              ...
              <h3 className="break-all text-lg font-bold text-[#1A0A10] md:text-xl">#{orderData.invoice_id}</h3>
            </div>
            <div className="text-right">
              <span className="inline-block bg-[#C2185B] px-4 py-2 text-xs font-bold uppercase tracking-widest text-white">
                    ...
                    <div
                      className={`flex h-9 w-9 items-center justify-center rounded-full ${
                        isCompleted
                          ? "border-2 border-[#C2185B] bg-[#C2185B] text-white"
                          : "border-2 border-[#F0D9E5] bg-white text-[#8D6E7F]"
                      } ${isCurrent ? "ring-2 ring-[#F0D9E5]" : ""}`}
                    >
                      <Icon className="h-4 w-4" />
                    </div>
                    {!isLast && <div className={`mt-1 h-8 w-0.5 ${isCompleted ? "bg-[#C2185B]" : "bg-[#F0D9E5]"}`} />}
                  </div>
          ...
          <div className="grid grid-cols-1 gap-8 border border-[#F0D9E5] bg-[#FDF6F8] p-6 md:grid-cols-2">
            <div>
          ...
          <div className="mt-8 border border-[#F0D9E5] bg-[#FDF6F8] p-6">
            ...
            <div className="space-y-3">
              {orderItems.length > 0 ? (
                orderItems.map((item, index) => {
                  ...
                  return (
                    <div key={`${productName}-${index}`} className="flex items-center gap-4 border border-[#F0D9E5] bg-white p-3">
                      <div className="relative h-16 w-16 shrink-0 overflow-hidden border border-[#F0D9E5] bg-[#FDF6F8]">
                        {imageSrc ? (
                      ...
                      <p className="text-sm font-bold text-[#1A0A10]">{TAKA_SYMBOL} {lineTotal.toLocaleString()}</p>
                    </div>
                  );
                })
              ) : (
