...
function formatApiCell(value, unit) {
...
export default function SizeChartModal({ isOpen, onClose, product }) {
  ...
  const usesApiChart = chartValues.length > 0;

  const { columns, rows, chartTitle, howToMeasure } = useMemo(() => {
    if (usesApiChart) {
      const potentialKeys = [
      ...
      const tableRows = chartValues.map((row) => ({
        ...
        values: activeKeys.map((k) => formatApiCell(row?.[k.key], unit)),
      }));
      ...
      return {
        columns: header,
        rows: tableRows,
        chartTitle: product?.size_chart_category?.name || "",
        howToMeasure: staticData.howToMeasure,
      };
    }

    const staticData = sizeChartData[productType] || sizeChartData.generic;
    const header = unit === "in" ? staticData.columns : staticData.columnsCm;
    const tableRows = staticData.rows.map((row) => {
      const values = Object.values(row).slice(1);
      const converted =
        unit === "cm"
          ? values.map((val) => (typeof val === "number" ? inchesToCm(val) : val))
          : values.map((val) => (typeof val === "number" ? String(val) : val));
      return { label: row.size, values: converted };
    });

    return {
      columns: header,
      rows: tableRows,
      chartTitle: staticData.name,
      howToMeasure: staticData.howToMeasure,
    };
  }, [chartValues, unit, productType, usesApiChart, product?.size_chart_category?.name]);
        ...
        <div className="flex h-full flex-col">
          <div className="flex items-center justify-between border-b border-[#E5E5E5] px-4 py-4">
            <h2 className="text-sm font-bold uppercase tracking-wide text-[#1A1A1A]">
              Size Chart{chartTitle ? ` - ${chartTitle}` : ""}
            </h2>
          ...
          <div className="flex-1 overflow-y-auto px-4 py-4">
            {activeTab === "chart" ? (
              <>
                ...
                {!usesApiChart && rows.length > 0 && (
                  <p className="mt-4 text-xs text-[#6B6B6B]">* Measurements may vary slightly based on fit type</p>
                )}
              </>
            ) : (
